...
test("formatStatusTable renders aligned rows", () => {
  const rows: ProfileState[] = [
    { kind: "sso" as const, name: "prod", status: "valid", expiresAt: "2026-06-11T13:00:00.000Z", ssoExpiresAt: null, ssoRenewable: false, favorite: true },
    { kind: "sso" as const, name: "staging", status: "needs-login", expiresAt: null,  ssoExpiresAt: null, ssoRenewable: false, favorite: false },
  ];
...
test("formatStatusTable keeps the marker column aligned across rows", () => {
  const rows: ProfileState[] = [
    { kind: "sso" as const, name: "prod", status: "valid", expiresAt: null,  ssoExpiresAt: null, ssoRenewable: false, favorite: true },
    { kind: "sso" as const, name: "dev", status: "valid", expiresAt: null,  ssoExpiresAt: null, ssoRenewable: false, favorite: false },
  ];
