...
import { useBodyHeight, useContentWidth } from "../components/App.js";
...
/** Rows the table frame costs: its border, plus the column header inside it. */
...
const TYPE_LABEL: Record<ProfileState["kind"], string> = {
  sso: "sso",
  assume: "assumed",
};
...
/**
 * Width the two-row legend needs. Below it the legend folds to the essentials
 * plus a pointer to `?`. Measured against its longest row — a legend that wraps
 * silently costs the table a row it has already committed to using.
 */
const FULL_LEGEND_WIDTH = 68;

/**
 * The full legend is folded to the essentials plus a pointer to `?` when it
 * would otherwise wrap into an unreadable run-together of half-hints.
 */
...
export function Dashboard(props: Props) {
  ...
  const compactLegend = contentWidth < FULL_LEGEND_WIDTH;
  const legendRows = compactLegend ? 1 : 2;
  // The table takes everything the body has, less its own status line and the
  // key hints pinned under it — so the frame reaches the bottom of the screen
  // whatever the terminal's height.
  const tableHeight = Math.max(TABLE_CHROME_ROWS + 1, bodyHeight - legendRows - 1);
  const capacity = Math.max(1, tableHeight - TABLE_CHROME_ROWS);
  ...
  if (showHelp) {
    return (
      <Box flexDirection="column" flexGrow={1}>
        <Help width={contentWidth} height={bodyHeight - 1} />
        <KeyBar>
    ...
    <Box flexDirection="column" flexGrow={1}>
      ...
      >
        ...
        <Box>
          ...
          {columns.showType && <Cell text="TYPE" width={W_TYPE} dim bold />}
        ...
        {page.map((p, i) => {
          ...
          return (
            <Box key={p.name}>
              ...
              {columns.showType && (
                <Cell text={TYPE_LABEL[p.kind]} width={W_TYPE} dim highlight={hi} />
              )}
              <Cell
      ...
      {/* One reserved row under the table: where you are in the list, or the
          filter, so neither can make the layout jump. */}
